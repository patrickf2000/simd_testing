...
void sum(double **matrix,double *vector,double **dest)
{
  ...
  for (int i = 0; i < 1200; i++) {
    ...
    for (j = 0; j <= 1199; j += 4) {
      ...
      __m256d __vec2 = _mm256_loadu_pd(&__ptr1[j]);
      __m256d __vec3 = _mm256_loadu_pd(&vector[j]);
      __m256d __vec4 = _mm256_mul_pd(__vec3,__vec2);
      _mm256_storeu_pd(&__ptr0[j],__vec4);
    }
  }
}
// Debug functions
