...
void sum(double **matrix,double *vector,double **dest)
{
  ...
  for (int i = 0; i < 1200; i++) {
    ...
    for (j = 0; j <= 1199; j += svcntd()) {
      double *__ptr0 = dest[i];
      double *__ptr1 = matrix[i];
      svfloat64_t __vec2 = svld1(__pg0,&__ptr1[j]);
      svfloat64_t __vec3 = svld1(__pg0,&vector[j]);
      svfloat64_t __vec4 = svmul_f64_m(__pg0,__vec3,__vec2);
      svst1(__pg0,&__ptr0[j],__vec4);
      __pg0 = svwhilelt_b64(j,1199);
    }
  }
}
// Debug functions
