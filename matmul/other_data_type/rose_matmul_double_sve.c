...
void matmul_simd(double **A,double **B,double **C)
{
  ...
  for (i = 0; i < 512; i++) {
    for (j = 0; j < 512; j++) {
      temp = 0;
      svbool_t __pg0 = svwhilelt_b64(0,511);
      svfloat64_t __vec0 = svdup_f64(temp);
      svfloat64_t __part7 = svdup_f64(0.00000);
      for (k = 0; k <= 511; k += svcntd()) {
        double *__ptr1 = A[i];
        svfloat64_t __vec2 = svld1(__pg0,&__ptr1[k]);
        double *__ptr3 = B[j];
        svfloat64_t __vec4 = svld1(__pg0,&__ptr3[k]);
        svfloat64_t __vec5 = svmul_f64_m(__pg0,__vec4,__vec2);
        svfloat64_t __vec6 = svadd_f64_m(__pg0,__vec5,__vec0);
        __part7 = svadd_f64_m(__pg0,__part7,__vec6);
        __pg0 = svwhilelt_b64(k,511);
      }
