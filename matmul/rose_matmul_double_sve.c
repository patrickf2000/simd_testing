...
void matmul_simd(double **A,double **B,double **C)
{
  ...
  for (i = 0; i < 1024; i++) {
    for (j = 0; j < 1024; j++) {
      temp = 0;
      svbool_t __pg0 = svwhilelt_b64(0,1023);
      for (k = 0; k <= 1023; k += svcntd()) {
        double *__ptr0 = A[i];
        svfloat64_t __vec1 = svld1(__pg0,&__ptr0[k]);
        double *__ptr2 = B[j];
        svfloat64_t __vec3 = svld1(__pg0,&__ptr2[k]);
        svfloat64_t __vec4 = svmul_f64_m(__pg0,__vec3,__vec1);
        temp += svaddv(__pg0,__vec4);
        __pg0 = svwhilelt_b64(k,1023);
      }
      C[i][j] = temp;
    }
  }
